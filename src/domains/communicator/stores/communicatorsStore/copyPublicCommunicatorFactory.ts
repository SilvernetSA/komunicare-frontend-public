...
const findCopiedCommunicator = (
  ...
  for (let index = communicators.length - 1; index >= 0; index -= 1) {
    ...
    if (String(communicator.publicSourceId || '') === sourceCommunicatorId) {
      return communicator;
    }
  }
...
export const copyPublicCommunicatorFactory =
  ...
  async (sourceCommunicatorId: string): Promise<Communicator> => {
    ...
    try {
      const { data } = await apiClient.post(
        `/communicator/public/${sourceCommunicatorId}/copy`,
      );
    ...
    try {
      ...
      if (existingCommunicatorId) {
        get().setApiFailure();
        throw new Error(
          `Communicator public copy conflict references missing communicator ${existingCommunicatorId}. Backend data may be inconsistent and may require DB cleanup.`,
        );
      }
    }
    ...
    get().setApiFailure();
    throw new Error(
      `Copied communicator ${sourceCommunicatorId} could not be resolved after refresh.`,
    );
  };
