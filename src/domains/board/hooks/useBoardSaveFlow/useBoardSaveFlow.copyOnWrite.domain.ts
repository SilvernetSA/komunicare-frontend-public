...
export const OFFICIAL_DEFAULT_COMMUNICATOR_ID = 'komunicare';
export const LEGACY_DEFAULT_COMMUNICATOR_ID = 'komunicare_default';
...
export const CANONICAL_ROOT_BOARD_ID_SET = new Set(CANONICAL_ROOT_BOARD_IDS);
...
const isOfficialDefaultCommunicator = (
  communicator?: Pick<Communicator, 'id'>,
): boolean =>
  String(communicator?.id || '').trim() === OFFICIAL_DEFAULT_COMMUNICATOR_ID;

const isLegacyDefaultCommunicatorSentinel = (
  communicator?: Pick<Communicator, 'id'>,
): boolean =>
  String(communicator?.id || '').trim() === LEGACY_DEFAULT_COMMUNICATOR_ID;
...
export const isProtectedCommunicator = (
  ...
  if (
    isOfficialDefaultCommunicator(communicator) ||
    isLegacyDefaultCommunicatorSentinel(communicator)
  ) {
    return true;
  }
...
export const buildCommunicatorCopy = (
  ...
  const copySourceCommunicatorId = String(
    (communicator as any).copySourceCommunicatorId ||
      (isLegacyDefaultCommunicatorSentinel(communicator)
        ? OFFICIAL_DEFAULT_COMMUNICATOR_ID
        : communicator.id) ||
      '',
  ).trim();
