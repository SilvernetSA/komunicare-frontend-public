...
describe('copyOnWrite protections', () => {
  ...
  it('treats the official default communicator as protected even if communicator email matches user', () => {
    const userData = { email: 'user@example.com', name: 'User' } as any;
    const communicator = {
      id: 'komunicare',
      email: 'user@example.com',
  ...
  it('keeps the legacy local default communicator sentinel protected', () => {
    const userData = { email: 'user@example.com', name: 'User' } as any;
    const communicator = {
  ...
  it('creates and activates a communicator copy through the shared helper', async () => {
    ...
    expect(dialogHandler).toHaveBeenCalledWith({
      communicator: expect.objectContaining({ id: 'komunicare' }),
      promptText: 'prompt',
  ...
  it('buildCommunicatorCopy sets copy source metadata from official communicator', () => {
    ...
    expect(copy.copySourceCommunicatorId).toBe('komunicare');
  });

  it('normalizes the legacy local default communicator source id when building copies', () => {
    const legacyDefaultCommunicator = {
      id: 'komunicare_default',
      email: 'support@komuni.care',
      name: 'Komunicare',
      rootBoard: 'komunicare',
