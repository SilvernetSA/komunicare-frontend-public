...
vi.mock(
  '@/domains/board/stores/boardsStore/persistBoardAndSyncNavigation',
  () => ({
...
const createCommunicatorCopyParams = (
  ...
  activeCommunicator: { id: 'communicator-id', email: 'owner@example.com' },
...
describe('saveProtectedBoardWorkflow', () => {
  ...
  it('prepares the board and persists with navigation sync using resolved copy state', async () => {
    ...
    const syncActiveBoardAfterSave = vi.fn();
