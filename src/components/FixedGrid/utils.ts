...
export function sortGrid({
  ...
  const sortedGrid = fillEmptyGridCells(grid, itemsToSort);
  return compact ? compactSortedGrid(sortedGrid, rows, columns) : sortedGrid;
}

function fillEmptyGridCells(grid: unknown[][], items: unknown[]) {
...
function compactSortedGrid(
  ...
  const compactGrid = createMatrix(rows, columns);
  let compactIndex = 0;
